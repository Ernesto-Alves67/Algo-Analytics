    ...
    , ui(new Ui::MainWindow)
{
    ...
    ui->te_entryDisplay->setCursor(Qt::ArrowCursor);
...
void MainWindow::btn_geraArvoreClicked()
{
    ...
    QString indice = QString::fromStdString(estruturas[MainWindow::LIST]);
    ui->te_entryDisplay->append("---------------------------------------------------------");
    ui->te_entryDisplay->append("Arvore Criada."+indice);

    int num_nos = ui->le_numNos->text().toInt();
    /// Criando Raiz
    TreeNode root(geraRandomNum(), true);
    num_nos--;
...
void MainWindow::btn_geraGrafoClicked()
{
    ...
    if (tipoGrafo == 0) {
        ...
        for (int i = 0; i < vertices; i++) {
            grafo.adicionarAresta(i, i + 1, geraRandomNum());
        }
...
void MainWindow::btn_deletar2Clicked()
{
    if(estruturas.empty()){
        ...
        int tipoEstrutura = ui->cb_tipoEstrutura->currentIndex();


        if(listas.empty() && arvores.empty() && grafos.empty()){
            ui->te_entryDisplay->append("---------------------------------------------------------");
            ui->te_entryDisplay->append("Nada a ser removido!!");
            return;
        }
        auto it = estruturas.find(MainWindow::LIST);
        if (it != estruturas.end() && !listas.empty() && tipoEstrutura == 0) {
            int qtd = stoi(estruturas[MainWindow::LIST]);
            qtd -= 1;
            estruturas[MainWindow::LIST] = to_string(qtd);
            listas.pop_back();
        ...
        if (it != estruturas.end() && !grafos.empty() && tipoEstrutura == 2) {
            int qtd = stoi(estruturas[MainWindow::GRAPH]);
            qtd -= 1;
            estruturas[MainWindow::GRAPH] = to_string(qtd);
            grafos.pop_back();
